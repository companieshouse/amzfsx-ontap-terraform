data "aws_iam_policy_document" "e5_nfs_fsx" {
  statement {
    actions = ["fsx:*",
      "ec2:Describe*",
      "ec2:CreateTags",
...
resource "aws_iam_role" "e5_nfs_fsx_role" {
  name = "e5_nfs_fsx_role"
...
resource "aws_iam_role_policy_attachment" "attach_e5_nfs_fsx_policy" {
  role       = aws_iam_role.e5_nfs_fsx_role.name
  policy_arn = aws_iam_policy.e5_nfs_fsx.arn
}